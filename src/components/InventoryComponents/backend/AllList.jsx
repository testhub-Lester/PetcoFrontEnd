...
function AllList_Inventory({userData}){
    ...
    useEffect(() => {
        ...
        axios.post(localStorage.getItem('urlLink')+'getInventoryListAllInventory.php', data)
            .then(response => {
    ...
    ): (<div class="inventoryList-empty"> <label>EMPTY</label> <p>- ALL LIST -</p> </div>)}
             {showInfo && <div key={key}><InformationTab closeModal={setshowInfo} userData={userData} itemID={itemInfoID} itemName={itemInfoName} itemIdentifier={itemInfoIdentifier}/></div>}
    </div>
    )
}
