...
function ModalPayment_Cashier({userData}) {
    ...
    const ToPrint = () => {
        if (inputPayment == null || inputPayment == 0) {
            alert('INPUT CORRECT PAYMENT AMOUNT')
        }
        else if (inputPayment < totalAmount)
        {
            alert('INPUT PAYMENT IS INSUFFICIENT')
    ...
    return (
        ...
        <td>
            ...
            <button onClick={()=> {AddQuantity(item.itemID, item.currentItemName, item.defaultPrice, item.currentItemPrice, item.currentItemQuantity, item.itemQuantity, item.currentItemDiscount, item.itemSold);         setKey(prevKey => prevKey + 1); navigate('/Cashier/PaymentTab')}}>+</button>
            </div>
        </td>
        <td>
